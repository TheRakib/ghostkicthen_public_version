...
const isCloseToBottom = ({ layoutMeasurement, contentOffset, contentSize }) => {
  return (
    ...
    layoutMeasurement.height + contentOffset.y < contentSize.height - 700
  );
};
...
class HomeScreen extends React.Component {
  constructor(props) {
    ...
    if (VendorAppConfig.isMultiVendorEnabled) {
      this.vendorAPIManager = new IMVendorListAPI(
        props.setVendors,
        VENDOR,
        this.onVendorListLoadingStateUpdate,
      );
      this.vendorAPIManager.fetchVendors();
    } else {
      this.productsAPIManager = new ProductsAPIManager(
        props.setPopularProducts,
      );
    }
    ...
    this.lastDataFetchTimeStamp = null;
  ...
  componentWillUnmount() {
    ...
    this.productsAPIManager?.unsubscribe &&
      this.productsAPIManager.unsubscribe();
  }
  ...
  onVendorListLoadingStateUpdate = (loadingState) => {
    this.setState({
      ...
      <ScrollView
        ...
        onScroll={({ nativeEvent }) => {
          ...
          if (
            ...
            this.lastDataFetchTimeStamp != currentTimestamp
          ) {
            this.vendorAPIManager.fetchVendors();
            this.lastDataFetchTimeStamp = currentTimestamp;
          }
        ...
        <Text style={styles.title}> {IMLocalized('Bestsellers')} </Text>
        ...
        <View style={styles.mostPopular}>
          <Text style={styles.title}> {IMLocalized('Food Artists')} </Text>
          {appConfig.isMultiVendorEnabled ? (
            <View>
              ...
              />
              {this.state.loading && (
                <View style={styles.loadingStateText}>
                  <LottieView
                    style={styles.lazyLoading}
                    source={require('../../CoreAssets/Lotties/lazyLoading.json')}
                    autoPlay
                    loop
                  />
                </View>
